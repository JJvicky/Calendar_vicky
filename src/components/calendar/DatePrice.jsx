...
class DatePrice extends Component {
    ...
    render() {
        ...
        const listArray = this.props.month_array.filter((item)=>{    //list array
            return item.price 
        })
     
        const clMode = this.props.calendarMode;
        const MonthArray = clMode ? this.props.month_array : listArray  //判斷甚麼模式來決定取哪個array
                        ...
                        <tbody>
                            ...
                            {MonthArray.map((item, index) => {
                               // const Index = this.props.prevEmpty.length+index;
                                ...
                                return (
                                        <td id={index} className={`${toggleId}`+''+this.inPage(index) } onClick={this.props.toggleItem} key={index} data-date={item.date}>
                                            ...
                                            <section className="middle">
                                               ...
                                               {clMode?<div className={item.date ? this.getStatusColor(item[status]) : "status disable"}>{item[status]}</div>:''} 
                                                ...
                                                <div className={item[guaranteed] ? 'guaranteed' : "guaranteed "}>{item[guaranteed]?"成團":''}</div>
                                            </section>
                                 ...
                                 {this.props.afterEmpty.map((item,id)=>{
                                     const Index = this.props.prevEmpty.length+this.props.month_array.length+id;
                                 return(
